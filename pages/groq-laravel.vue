    ...
    <div class="container h-full w-full flex flex-row gap-12 mx-auto py-24 px-6 lg:px-4 relative">
        <div class="flex flex-col w-full max-w-80">
            <div class="sticky top-20">
                <NuxtLink to="/" class="text-orange-500 hover:underline">&larr; Voltar</NuxtLink>
                <h2 class="text-5xl font-black mt-4 mb-8">Groq-Laravel</h2>
                <!-- <p class="mb-6">Pacote Laravel para integração com a API Groq.</p> -->
                ...
                <div class="flex items-center mt-2">
                    <select v-model="selectedVersion" @change="loadVersion" id="version-select"
                        class="form-select mt-1 block w-full py-2 px-3 border border-gray-300 bg-white text-black rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        <option v-for="version in releases" :key="version" :value="version">{{ version }}</option>
                    </select>
                    <a :href="`https://github.com/lucianotonet/groq-laravel/releases/${selectedVersion}`"
                        target="_blank" class="ml-4 text-gray-500 hover:text-gray-700" title="Ver no GitHub">
                ...
                <nav v-if="anchors.length" class="sidebar mt-8">
                    ...
                    <ul>
                        <li v-for="anchor in anchors" :key="anchor.id"
                            :style="{ marginLeft: (anchor.level - 1) * 15 + 'px' }" class=" leading-tight">
                            <a :href="`#${anchor.id}`" class="text-orange-500 hover:underline text-sm leading-none"
...
const fetchVersions = async () => {
  ...
  try {
    const response = await fetch('https://api.github.com/repos/lucianotonet/groq-laravel/tags', {
      headers: {
    ...
    if (!response.ok) {
      if (response.status === 403) {
        ...
        console.error('Limite de requisições da API excedido. Consulte a documentação para mais detalhes sobre limites de requisições.');
      }
    ...
    selectedVersion.value = versionFromUrl || releases.value[0] // Define a versão da URL ou a última versão como padrão
...
// Watch selectedVersion to update URL and fetch readme
