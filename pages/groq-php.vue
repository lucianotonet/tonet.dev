      ...
      <div class="sticky top-20">
        ...
        <div class="flex items-center">
          <select v-model="selectedVersion" @change="loadVersion" id="version-select"
            ...
            <option value="main">Next Release</option>
            <option v-for="version in releases" :key="version" :value="version">{{ version }}</option>
          </select>
          <a :href="`https://github.com/lucianotonet/groq-php/tree/${selectedVersion}`" target="_blank"
...
const fetchVersions = async () => {
  try {
    const response = await $fetch('api/github/tags/lucianotonet/groq-php.json');
